...
def _ccov_integrand_LELP(params, r_grid, LLp_grid, LLx_grid, LEp_B_grid, LEx_B_grid, EP_BD_grid, LP_D_grid):
    ...
    # Geometry (computed once for both components) - Direct Cartesian calculation
...
def generate_ccov_LELP(B, D):
    ...
    _, EP_BD_grid = spline_to_grid(EP[B][D], 0, r_grid_max, n_points=n_grid_points)
...
def generate_ncov_LELP(B, D):
    ...
    n_grid_points = 5000
    r_grid_max = min(3 * r2_max, Thetamax)
    r_grid, EP_BD_grid = spline_to_grid(EP[B][D], 0, r_grid_max, n_points=n_grid_points)
